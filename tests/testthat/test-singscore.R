...
test_that("test run_singscore with dorothea genesets", {
  res1 =  run_singscore(emat, tiesMethod="min", dorothea_genesets, .source="tf",
                        .target="target", .target_profile="mor", minsize=0,
                        perm=100, ncores=6, directed=TRUE, tidy=FALSE)
  exp1 = readRDS(
  ...
  res2 =  run_singscore(emat, tiesMethod="min", dorothea_genesets, .source="tf",
                        .target="target", .target_profile="mor", minsize=0,
                        perm=100, ncores=6, directed=TRUE, tidy=TRUE)
  exp2 = readRDS(
  ...
  res3 =  run_singscore(emat, tiesMethod="min", dorothea_genesets, .source="tf",
                        .target="target", .target_profile="mor", minsize=4,
                        perm=100, ncores=6, directed=TRUE, tidy=FALSE)
  exp3 = readRDS(
  ...
  res4 =  run_singscore(emat, tiesMethod="average", dorothea_genesets, .source="tf",
                        .target="target", .target_profile="mor", minsize=0,
                        perm=100, ncores=6, directed=TRUE, tidy=FALSE)
  exp4 = readRDS(
    system.file("testdata/outputs/singscore/", "output-singscore_dorothea_avg.rds",
                package = "decoupleR")
  )
...
test_that("test run_singscore with regnetwork genesets", {
  res1 =  run_singscore(emat, tiesMethod="min", regnetwork_genesets, .source="tf",
                        .target="target", .target_profile=NULL, minsize=0,
                        perm=100, ncores=6, directed=FALSE, tidy=FALSE)
  exp1 = readRDS(
  ...
  res2 =  run_singscore(emat, tiesMethod="min", regnetwork_genesets, .source="tf",
                        .target="target", .target_profile=NULL, minsize=0,
                        perm=100, ncores=6, directed=FALSE, tidy=TRUE)
  exp2 = readRDS(
  ...
  res4 = run_singscore(emat, tiesMethod="average", regnetwork_genesets, .source="tf",
                      .target="target", .target_profile=NULL, minsize=0,
                      perm=100, ncores=6, directed=FALSE, tidy=FALSE)

  exp4 = readRDS(
    system.file("testdata/outputs/singscore/", "output-singscore_regnetwork_avg.rds",
                package = "decoupleR")
  )
