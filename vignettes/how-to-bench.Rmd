...
The benchmarking results provides meta information about the run: set_name (name of the set source), bench_name (name of the benchmark dataset), filter_crit (filter criteria), and statistic (statistical method).
...
It is a rather lengthy, yet highly informative table, containing informations such as:
meta information for each run, AUROC (auc), Precission-Recall Curve AUC (pr_auc), source_cov (number of TFs), condition_cov (number of experiments), statistic_time (Computational time for the statistical method), regulon_time (combined time for each of the statistics for each network/set_source).
